...
public abstract class AbstractAdvisorAutoProxyCreator extends AbstractAutoProxyCreator {
	...
	protected Object[] getAdvicesAndAdvisorsForBean(
		...
		// TODO 切面（Advisors）：包含Pointcut（就是那个Aspect表达式）和Advice（before、after、afterReturn、AfterThrowing等方法）
		//找到合格的切面，也就是找到可以拦截到当前bean的切面，其实就是一个寻找有@Aspectj注解的过程,找到能够拦截到当前Bean的切面封装为对象返回
	...
	protected List<Advisor> findEligibleAdvisors(Class<?> beanClass, String beanName) {
		//找到所有的切面,其实就是一个寻找有@Aspectj注解的过程，把工程中所有有这个注解的类封装成Advisor返回
		...
		if (!eligibleAdvisors.isEmpty()) {
			//对有@Order@Priority进行排序，对切面进行排序，@Around>@Before>@AfterReturn>AfterThrow>@After
	...
	protected List<Advisor> findAdvisorsThatCanApply(
		...
		try {
			//看看当前类是否在这些切面的pointCut中，候选的切面对象和当前Bean的匹配过程，本质就是拿到能够拦截到当前Bean的切面
